...
- **Dependencies:** compute.py, log_analysis.py (subprocess)
- **Usage:** `python gui_main.py`

### Testing/Alternative Tools

#### `SubsituteMemoryBufferCpp.py`
...
- **Dependencies:** compute.py
- **Usage:** `python SubsituteMemoryBufferCpp.py`
...
- **Usage:**
  ```bash
  python log_analysis.py <log_path_or_pattern> [-o output_dir]
  # Example: python log_analysis.py "../Cpp/logs/log*" -o analysis
  ```
- **Called by:** gui_main.py (automatically on exit)
...
- **Usage:**
  ```bash
  python visualize_positions.py <log_file>        # Show last snapshot
  python visualize_positions.py <log_file> --all  # Show all snapshots
  ```

#### `PositionErrorCalculation.py`
...
- **Usage:** `python PositionErrorCalculation.py`
...
compute.py
    ...
    │       └─> log_analysis.py (called via subprocess on exit)
    └─> SubsituteMemoryBufferCpp.py (Testing tool)
...
```bash
python SubsituteMemoryBufferCpp.py
```
...
```bash
# Windows
python log_analysis.py "..\Cpp\logs\log*"
...
```bash
python PositionErrorCalculation.py
```
...
```bash
# Windows
python visualize_positions.py ..\Cpp\logs\log_20241201.txt
